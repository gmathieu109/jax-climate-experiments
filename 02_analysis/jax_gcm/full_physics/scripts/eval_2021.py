...
def plot_bias(ds_era5, ds_jcm_raw, ds_jcm, jcm_is_clim, out_path):
    ...
    if jcm_is_clim:
        ...
        for ax, region in zip(axes, regions):
            ...
            era, jcm = xr.align(era, jcm, join="inner")
            bias = jcm - era
...
def plot_bias_map(da_era5_field, da_jcm_field, out_path):
    ...
    # Align both fields to common dates
    da_era5_overlap, da_jcm_overlap = xr.align(
        da_era5_field,
        da_jcm_field,
        join="inner",
    )

    if len(da_era5_overlap.time) == 0 or len(da_jcm_overlap.time) == 0:
