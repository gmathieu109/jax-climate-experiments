#!/usr/bin/env python3
"""
Chunked JCM DRY simulation using ERA5 data.

Usage:
    ~/extras/.conda/envs/jcm/bin/python run_jcm_era5_dry.py
    ~/extras/.conda/envs/jcm/bin/python run_jcm_era5_dry.py --total_days 30 --chunk_days 10
"""
...
def build_initial_state(era5_ds, coords, init_time):
    """Interpolate ERA5 onto JCM grid and build an initial state."""
...
def main():
    parser = argparse.ArgumentParser(description="Chunked JCM DRY simulation")
    ...
    parser.add_argument("--output", default="jcm_dry_run.nc",
    ...
    for i in range(n_chunks):
        ...
        if "temperature" in ds_chunk:
            tmin = float(ds_chunk["temperature"].min())
            tmax = float(ds_chunk["temperature"].max())
            n_nan = int(ds_chunk["temperature"].isnull().sum())
            print(f"   Saved → {chunk_file}  "
                  f"(T range: {tmin:.1f}–{tmax:.1f} K, NaN: {n_nan})\n")
        else:
            print(f"   Saved → {chunk_file}\n")
