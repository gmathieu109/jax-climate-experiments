...
def plot_comparison(
    ...
    if jcm_is_clim:
        ...
        for region in region_names:
            ...
            global_min = min(
                global_min,
                np.nanmin(era5_overlap[region].values),
                np.nanmin(ds_baseline[region].values),
                np.nanmin(ds_optimized[region].values),
            )

            global_max = max(
                global_max,
                np.nanmax(era5_overlap[region].values),
                np.nanmax(ds_baseline[region].values),
                np.nanmax(ds_optimized[region].values),
            )
        ...
        for ax, region in zip(axes, region_names):
            ...
            ax.plot(
                ds_baseline.time.values,
                ds_baseline[region].values,
                color="firebrick",
                lw=1.5,
                label="Baseline",
            )
            ...
            ax.plot(
                ds_optimized.time.values,
                ds_optimized[region].values,
                color="royalblue",
                lw=1.5,
                label="Optimized",
            )
       ...
       fig.suptitle(
            f"ERA5 vs Baseline vs Optimized — {ERA5_LEVEL_HPA} hPa Temperature",
            fontsize=13,
            fontweight="bold",
            y=1.01,
        )
