...
def plot_bias_map(da_era5_field, da_jcm_field, out_path, title):
    ...
    bias = (jcm_mean - era_mean).transpose("lat", "lon")

    global_bias = float(bias.mean())

    weights = np.cos(np.deg2rad(bias.lat))
    mean_bias = bias.weighted(weights).mean(("lat", "lon")).item()
    ...
    cbar = plt.colorbar(im, ax=ax, shrink=0.88, pad=0.03)
    cbar.set_label("Temperature bias (K)", fontsize=11)
    ...
    ax.text(
        0.015, 0.045,
        f"Global mean bias = {mean_bias:.2f} K",
    ...
    print(f"Global mean bias = {mean_bias:.3f} K")
    plt.close(fig)
...
def main():
    ...
    plot_bias_map_seasonal(
        ...
        title="Mean temperature bias at 850 hPa (DJF)",
    )
    ...
    plot_bias_map_seasonal(
        ...
        title="Mean temperature bias at 850 hPa (JJA)",
    )
