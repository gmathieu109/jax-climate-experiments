...
def main():
    ...
    parser.add_argument(
        "--delta_sst_path",
        default=None,
        help="Path to delta_sst_final.npy to add to prescribed SST",
    )
    ...
    if args.delta_sst_path is not None:
        print(f"Applying delta SST from: {args.delta_sst_path}")

        delta_sst = np.load(args.delta_sst_path)
        delta_sst = jnp.asarray(delta_sst)

        sst = realistic_forcing.sea_surface_temperature

        print("Original SST shape:", sst.shape)
        print("Delta SST shape:", delta_sst.shape)

        if delta_sst.shape != sst.shape[:2]:
            raise ValueError(
                f"delta_sst shape {delta_sst.shape} does not match SST spatial shape {sst.shape[:2]}"
            )

        sst_new = sst + delta_sst[:, :, None]

        realistic_forcing = realistic_forcing.replace(
            sea_surface_temperature=sst_new
        )

        print("Delta SST applied.")
