...
def extract_config_from_dirname(dirname: str) -> dict:
    ...
    for i, part in enumerate(parts):
        if part.endswith("d"):
            ...
            config["lr"] = float(part[2:].replace("p", "."))
        elif part == "l2" and i + 1 < len(parts):
            config["lambda_l2"] = float(parts[i + 1].replace("p", "."))
        elif part == "sm" and i + 1 < len(parts):
            config["lambda_smooth"] = float(parts[i + 1].replace("p", "."))
        elif part == "reg":
...
def plot_heatmap(df: pd.DataFrame):
    ...
    sub = sub.dropna(subset=["lr", "lambda_smooth", "rmse_improvement_%"])
